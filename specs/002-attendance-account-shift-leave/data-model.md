...
- 範圍內：帳號、年假、班表／派班、出勤匯入（**伺服器正式匯入**）、政府日曆、工作地點詞庫、session、稽核。
- 範圍外：加班計算結果表（時數仍由前端線 A 計算）；SSO；未登入之本機公開上傳（不寫入本模型，見 PRD §5.6.1）。
- Session 用途：行事曆、Admin CRUD、伺服器匯入／詞庫等 API；**不是**進入加班單首頁的前置條件（PRD §5.3.1）。
