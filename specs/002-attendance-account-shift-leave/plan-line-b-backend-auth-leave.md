...
| **B7** 假勤摘要（PRD §5.5.1） | E3-4.4～4.5、E3-6 | 行事曆頁：曆年年假額度／已請／剩餘＋各假別已請加總；員工本人、Admin 可查任一員工 |
...
2. **本機上傳**：`FileUploader` 未登入可上傳；勾選「同時寫入伺服器」（預設勾選）→ 本機先算加班，**不強制導向登入**；之後登入同一檔呼叫 `POST /api/attendance/import`；失敗顯示原因、本機列表保留。
...
7a. **假勤摘要（B7，PRD §5.5.1）**：同一行事曆入口顯示選定曆年之年假額度／已請／剩餘，以及 §7.6 其他假別已請加總；可見性同行事曆；僅統計伺服器台帳。
...
- [ ] 首頁有明確行事曆入口；未登入點入 → 登入 → 顯示行事曆與假勤摘要。
- [ ] 假勤摘要：年假額度／已請／剩餘；各非年假假別已請加總；員工僅本人、Admin 可查任一員工。
- [ ] Admin 維護／查詢需登入。
