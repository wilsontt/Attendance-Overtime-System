...
- **📊 多元報表輸出**：
  ...
  - **固定版面**：每頁固定 16 列（表頭 1 列 + 資料 15 列），不足 15 筆以空白列補齊；頁面邊界與區塊間距依 `0.standards/輸出列印字體放大設計.md`（上 10mm／下 6mm／左右 10mm、區塊間距 8px、雙標題間距 4px、頁尾由下往上頁次 32px＋簽名 96px、列表 16 列等高均分）
