...
- 版號參考依據為 `frontend/package.json` 第 1–5 行（例如：`"version": "1.5.5"`）。
...
### 3a. 缺卡補登與請假
- 缺卡列可補登上下班時間與缺卡備註；下載前三者皆須完整
- 缺卡備註串到匯出備註欄，不自動寫入加班原因
- 請假天數 ≥ 1：鎖定補登與加班原因，時數 0；< 1：視同須出勤

### 4. 預覽與驗證（更新）

點擊「下載」按鈕後開啟預覽視窗：

- **門檻過濾**：僅保留完整刷卡且 `overtimeHours >= 0.5` 的記錄
- **記錄選擇**：勾選要包含在報表中的記錄
- **假日↔平日切換**：與主列表「加到例假日加班／加到平日加班」雙向同步
- **工作地點填寫**：開啟時游標停在平日工作地點（無平日則例假日）
- **鎖定式 Modal**：點外框不關閉，僅取消或 ×
- **加班原因編輯**：填寫每筆記錄的加班原因
- **ITEM 序號欄**：預覽表每筆記錄最前方顯示 ITEM（1,2,3...）
- **自動分類**：自動分為「平日加班」與「例假日加班」兩個表格（含員工編號＋姓名）
- **頁碼顯示**：每個表格區塊顯示頁碼
- **必填與格式驗證**：
  - 被選中且需填寫原因的記錄，不可為空
...
- **列表字級**：出勤記錄列表字級為 **14px**
- **欄位顯示規則**：
  - 日期欄採 2 列顯示（日期 + 星期與段別，如 `週三 早段`、`週三 晚段`）
...
### v1.5.5 (2026-09-03)
- 缺卡補登、全天／部分請假、主列表平假日覆寫
- 預覽門檻過濾、自動聚焦、鎖定關閉
- TXT 上下班對位、列尾「符合」、12:00 備用分配
- `0.shared-ui` 頂欄與 Docker COPY 路徑
