...
- **固定版面規格**：
  ...
  - 不足 15 筆資料時，自動補空白列
  - **頁面邊界與區塊間距**（與 `0.standards/輸出列印字體放大設計.md` 一致）：上邊界 10mm、下邊界 6mm、左右 10mm；上方區與列表、列表與下方簽名區各 **8px**；公司標題與表單標題間 **2px**；表單標題與員工姓名區間 **5px**；頁尾由下往上為 **頁次 32px**、**簽名區 96px**；列表 **16 列全部等高均分（各 100/16 ≈ 6.25%，套在 `<tr>`，不使用 `min-height`）**
